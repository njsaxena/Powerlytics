-- Feature Engineering SQL for EcoGridIQ ML Models
-- Creates engineered features for time series forecasting and anomaly detection

-- Hourly aggregation with rolling statistics
CREATE OR REPLACE TABLE `{project_id}.ecogrid_dwh.analytics_hourly` AS
WITH hourly_base AS (
  SELECT 
    device_id,
...
CREATE OR REPLACE TABLE `{project_id}.ecogrid_dwh.analytics_daily` AS
WITH daily_base AS (
  SELECT 
    device_id,
...
CREATE OR REPLACE TABLE `{project_id}.ecogrid_dwh.ml_features` AS
WITH base_features AS (
  ...
  FROM `{project_id}.ecogrid_dwh.raw_readings` r
...
CREATE OR REPLACE MATERIALIZED VIEW `{project_id}.ecogrid_dwh.realtime_anomalies` AS
SELECT 
  device_id,
  timestamp,
  power_w,
  ...
  power_rolling_avg_24h,
  power_rolling_std_24h
FROM `{project_id}.ecogrid_dwh.ml_features`
WHERE timestamp >= TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 24 HOUR)
  AND anomaly_score > 1.5
ORDER BY timestamp DESC;

-- Create view for model training data
CREATE OR REPLACE VIEW `{project_id}.ecogrid_dwh.training_data` AS
SELECT 
  device_id,
  timestamp,
  power_w,
