-- EcoGridIQ BigQuery Data Warehouse Schema
-- Creates tables for raw data, analytics, and ML features

-- Create dataset
CREATE SCHEMA IF NOT EXISTS `{project_id}.ecogrid_dwh`
OPTIONS (
  description = "EcoGridIQ Energy Analytics Data Warehouse",
  location = "US"
);

-- Raw readings table (partitioned by date, clustered by device_id)
CREATE OR REPLACE TABLE `{project_id}.ecogrid_dwh.raw_readings` (
...
-- Anomalies table
CREATE OR REPLACE TABLE `{project_id}.ecogrid_dwh.anomalies` (
  device_id STRING NOT NULL,
  timestamp TIMESTAMP NOT NULL,
...
CREATE OR REPLACE TABLE `{project_id}.ecogrid_dwh.analytics_hourly` (
  device_id STRING NOT NULL,
...
CREATE OR REPLACE TABLE `{project_id}.ecogrid_dwh.analytics_daily` (
  device_id STRING NOT NULL,
...
CREATE OR REPLACE TABLE `{project_id}.ecogrid_dwh.device_metadata` (
  device_id STRING NOT NULL,
  device_name STRING,
  device_type STRING,
  location STRING,
...
CREATE OR REPLACE TABLE `{project_id}.ecogrid_dwh.ml_features` (
  device_id STRING NOT NULL,
  timestamp TIMESTAMP NOT NULL,
  power_w FLOAT64,
  energy_wh FLOAT64,
...
CREATE OR REPLACE TABLE `{project_id}.ecogrid_dwh.forecasts` (
  device_id STRING NOT NULL,
...
CREATE OR REPLACE TABLE `{project_id}.ecogrid_dwh.recommendations` (
  device_id STRING NOT NULL,
...
CREATE OR REPLACE VIEW `{project_id}.ecogrid_dwh.current_usage` AS
SELECT 
  device_id,
  power_w,
  energy_wh,
...
FROM `{project_id}.ecogrid_dwh.raw_readings`
WHERE DATE(timestamp) = CURRENT_DATE();

CREATE OR REPLACE VIEW `{project_id}.ecogrid_dwh.device_summary` AS
SELECT 
  d.device_id,
  d.device_name,
  d.device_type,
...
FROM `{project_id}.ecogrid_dwh.device_metadata` d
LEFT JOIN `{project_id}.ecogrid_dwh.current_usage` c 
  ON d.device_id = c.device_id AND c.rn = 1
LEFT JOIN `{project_id}.ecogrid_dwh.analytics_hourly` h
  ON d.device_id = h.device_id 
  AND h.hour_ts >= TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 24 HOUR)
  AND h.hour_ts = (
    SELECT MAX(hour_ts) 
    FROM `{project_id}.ecogrid_dwh.analytics_hourly` h2 
    WHERE h2.device_id = h.device_id
  );

-- Insert sample device metadata
INSERT INTO `{project_id}.ecogrid_dwh.device_metadata` 
...
VALUES
('device_001', 'Main House Panel', 'electrical_panel', 'main_floor', '2023-01-15', 5000.0, 0.95, 'EcoGrid', 'EG-Panel-5000', 'active', CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP()),
('device_002', 'Garage Workshop', 'workshop', 'garage', '2023-02-01', 2000.0, 0.90, 'EcoGrid', 'EG-Workshop-2000', 'active', CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP()),
('device_003', 'HVAC System', 'hvac', 'basement', '2023-01-20', 3000.0, 0.88, 'EcoGrid', 'EG-HVAC-3000', 'active', CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP()),
('device_004', 'Kitchen Appliances', 'appliances', 'kitchen', '2023-03-01', 1500.0, 0.92, 'EcoGrid', 'EG-Kitchen-1500', 'active', CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP()),
('device_005', 'Smart Lighting', 'lighting', 'throughout', '2023-02-15', 800.0, 0.98, 'EcoGrid', 'EG-Lighting-800', 'active', CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP());
