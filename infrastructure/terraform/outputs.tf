...
output "service_account_email" {
  ...
  value       = google_service_account.powerlytics_backend.email
}
...
output "deployment_info" {
  ...
  backend_url  = google_cloud_run_service.powerlytics_backend.status[0].url
  dataset      = google_bigquery_dataset.powerlytics_dwh.dataset_id
    region       = var.region
